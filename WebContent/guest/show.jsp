...
$(function(){
	if(textId!=""){
		$.ajax({
			url:'<%=basePath %>guest.do/selectOneText.do',
			type:'post',
			data:{id:textId},
			dataType:'json',
			success:function(data){
				...
				$('#showTitle p').html("<li><small>作者:<a href='<%=basePath %>guest.do/showUser.do?userId="+data.userId+"'>"+data.userName+"</a></small></li><li class='text-muted'>|</li>"+
...
function showPinglunList(id){
	$.ajax({
		url:'<%=basePath %>guest.do/selectPinglunByTextId.do',
		type:'post',
		data:{id:id},
		dataType:'json',
		success:function(data){
			...
			$.each(data,function(i,n){
				huifukuang ="<div class='panel panel-primary' id='panel"+n.id+"' style='display:none;margin:5px 0;'><div class='panel-heading'>"+
							...
							"</div></div>";
				neirong ="<div class='panel-body text-left'>"+n.text;
...
function selectHuifu(NO){
	var huifuHtml = "";
	$.ajax({
		url:'<%=basePath %>guest.do/selectPinglunByNO.do',
		type:'post',
		data:{id:NO},
		dataType:'json',
		async:false,
		success:function(data){
			$.each(data,function(i,n){
				...
				huifuHtml= huifuHtml+"<div class='panel panel-primary' style='margin:5px 0;'><div class='panel-heading'>"+
						   ...
						   "<div class='panel-body text-left' style='height:80px;'>"+n.text+"</div></div>";
			});
		}
	})
	return huifuHtml;
	...
	<div class="cn-body container" >
		...
		<div id="showDiv">
			<div id="showTitle">
				...
				<p class="list-inline text-center"></p>
			</div>
		...
		<div class="text-center" style="width:100%;margin-bottom:20px;">
			...
			<button type="button" style="width: 80px;height: 40px;" onclick="showPinglun('pinglun',1)" class="btn btn-danger">评论</button>
		</div>
